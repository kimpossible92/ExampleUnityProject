...
public class DogController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        #region Rabbit
        if (findClosestResourceWithTag("Rabbit") != null)
        {
            if (Vector3.Distance(transform.position,
                findClosestResourceWithTag("Rabbit")
                .transform.position) <= distance)
            {
                if (findClosestResourceWithTag("Rabbit") != null)
                {
                    target = findClosestResourceWithTag("Rabbit").transform.position;
                    if (target != null)
                    {
                        agent.SetDestination(target);
                        if (Vector3.Distance(transform.position, target) <= 2f)
                        {
                            Destroy(findClosestResourceWithTag("Rabbit"));
                            findClosestResourceWithTag("Rabbit").GetComponent<Resource>().isOccupied = true;
                            switcher.SetController();
                            currentState = State.Wander;
                        }
                    }
                }
